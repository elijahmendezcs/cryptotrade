...
export default function DashboardPage() {
  return (
    <div className="min-h-screen flex bg-background text-foreground">
      {/* Sidebar */}
      <aside className="w-60 p-6 border-r border-muted">
        ...
        <nav className="flex flex-col space-y-2">
          ...
          <NavItem href="/backtest">Backtest</NavItem>
...
function NavItem({ href, active, children }: { href: string; active?: boolean; children: ReactNode }) {
  return (
    <Link
      href={href}
      className={`block px-3 py-2 rounded font-medium hover:bg-accent hover:text-accent-foreground ${
...
function StatusIndicator({ status }: { status: 'running' | 'paper' | 'stopped' }) {
  const color = status === 'running' ? 'bg-green-500' : status === 'paper' ? 'bg-yellow-500' : 'bg-red-500';
  const label = status === 'running' ? 'Running' : status === 'paper' ? 'Paper' : 'Stopped';
  return (
